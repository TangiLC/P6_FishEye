...
function getUserCardDOM (dataGal, firstName, slide_Nb) {
  ...
  let attribs = {class: 'content_card',ariaLabel: `galerie de ${artistFirstName}`};
  ...
  if (dataGal.hasOwnProperty('image')) {
    ...
    let attributes = {src: imgContentPath, alt: `Photo de ${artistFirstName} nommée ${title}`, tabindex: '0',
    ...
    let attributes = {class: 'galerieVideo', width: '350px', height: '300px',
          muted: 'true', alt: `Vidéo de ${artistFirstName} nommée ${title}`, tabindex: '0',
          onclick: `openLightbox();currentSlide(${slide_Nb})`,
          onkeydown: 'if(event.keyCode == 13){event.target.click()}'};
